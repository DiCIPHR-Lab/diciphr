...
wrapped into diciphr by Drew Parker (parkerwi@pennmedicine.upenn.edu)
"""
import pandas as pd
import patsy
import sys
...
def covbat(data, batch, model=None, numerical_covariates=None, pct_var=0.95, n_pc=0, return_estimates=False):
    ...
    return_estimates : bool
        If True, returns (covbat_data, gamma_hat, gamma_star, delta_hat, delta_star)
    ...
    """
    logging.debug('diciphr.statistics.harmonization.covbat')
    logging.info('CovBat: Correction of Covariance When Combating Batch Effects')
    ...
    if return_estimates:
        return x_covbat, gamma_hat, gamma_star, delta_hat, delta_star
    else:
        return x_covbat

def combat(data, batch, model=None, numerical_covariates=None, eb=True, return_estimates=False):
    ...
    corrected : pandas.DataFrame
        A (n_features, n_samples) dataframe of the batch-corrected data
    """
    logging.debug('diciphr.statistics.harmonization.combat')
    logging.info('ComBat: Combating Batch Effects when Combining Data')
    if isinstance(numerical_covariates, str):
        numerical_covariates = [numerical_covariates]
    if numerical_covariates is None:
        numerical_covariates = []
    ...
    if return_estimates:
        return bayesdata, gamma_hat, gamma_star, delta_hat, delta_star
    else:
        return bayesdata
